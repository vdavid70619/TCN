...
def create_fg_net():
    ...
    """
    name: 'nn'
    layer { name: "data" type: "DummyData" top: "data" dummy_data_param { shape { dim: 1 dim: 1 dim: 1 dim: 1024 } } }
    layer { name: "label" type: "DummyData" top: "label" dummy_data_param { shape { dim: 1 dim: 1 dim: 1 dim: 1 } } }

    layer { name: 'ip' type: 'InnerProduct' bottom: 'data' top: 'ip1' inner_product_param { num_output: 1000  weight_filler {type: "xavier"} } }
...
def create_deploy():
    ...
    layer { name: 'ip' type: 'InnerProduct' bottom: 'data' top: 'ip1' inner_product_param { num_output: 1000 } }
    layer { name: 'ip2' type: 'InnerProduct' bottom: 'ip1' top: 'ip2' inner_product_param { num_output: 201 } }
...
def classify_video(model, subset='validation', ANNOTATION = 'actNet200-V1-3.pkl', FEATURE = '/home/EXTRA/DATASETS/actnet/shuffle_imagenet/'):
    nf = create_deploy()
    net = caffe.Net(nf, model, caffe.TEST)

    with open(ANNOTATION, 'rb') as f:
        gt = pickle.load(f)['database']
    with h5py.File(FEATURE + 'ImageNetShuffle2016_features.h5', 'r') as f:
        mbh = np.asarray(f['features'])
    with open(FEATURE + 'ImageNetShuffle2016_quids.txt', 'r') as f:
        lines = f.readlines()
        idmap = {}
        for line in lines:
            temp = line.split(',')
...
if __name__ == '__main__':
    ...
    import logging
    logging.basicConfig(format='%(asctime)s %(message)s', filename='train_chunk_imshuffle'+time.strftime('_%Y_%m_%d.log'), level=logging.DEBUG)
    logging.getLogger()
    logging.info(' 201 classifier with imagenetshuffle')

    FEATURE = os.environ['ACTNET_HOME'] + '/shuffle_imagenet/'
